    ...
    try {
      ...
      for (var inv in invoices) { 
        ...
        for (var item in inv.items) {
          ...
          if (cost <= 0) {
            final found = productProvider.findProductAndVariantByVariantId(item.productId);
            cost = found?['variant']?.purchasePrice ?? 0.0;
          }
      ...
      for (var invoice in _salesInvoices) {
        for (var item in invoice.items) {
          ...
          List<ex.CellValue> rowData = [
            ...
            ex.TextCellValue(found?['product']?.category ?? 'غير معروف'),
