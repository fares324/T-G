...
class _ProvisionalSaleItem {
  final Product parentProduct;
  final ProductVariant variant;
  int quantity;

  _ProvisionalSaleItem({
  ...
  double get unitPrice => variant.sellingPrice;
...
class _NewSaleInvoiceScreenState extends State<NewSaleInvoiceScreen> {
  ...
  final List<_ProvisionalSaleItem> _invoiceItems = [];
  ...
  void _addItemToInvoice(_ProvisionalSaleItem newItem) {
    int existingIndex = _invoiceItems.indexWhere((item) => item.variant.id == newItem.variant.id);
    
    if (existingIndex != -1) {
      int newTotalQuantity = _invoiceItems[existingIndex].quantity + newItem.quantity;
      if (newTotalQuantity > newItem.variant.quantity) {
         ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('لا يمكن إضافة المزيد. الكمية المتوفرة لهذا المتغير هي ${newItem.variant.quantity}.', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo'))),
        );
        return;
      }
      setState(() { _invoiceItems[existingIndex].quantity = newTotalQuantity; });
    } else {
       if (newItem.quantity > newItem.variant.quantity) {
         ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('الكمية المطلوبة (${newItem.quantity}) أكبر من المتوفر بالمخزون (${newItem.variant.quantity}).', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo'))),
          );
          return;
      }
  ...
  void _updateItemQuantity(int index, int newQuantity) {
    ...
    if (newQuantity > item.variant.quantity) {
      if(mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('لا يمكن تحديد هذه الكمية. الكمية المتوفرة هي ${item.variant.quantity}', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo'))),
      );
      }
      return;
    }
  ...
  Future<void> _showProductSelectionDialog() async {
    ...
    final Product? selectedProduct = await showDialog<Product>(
      context: context,
      builder: (ctx) => _ProductPickerDialog(productProvider: productProvider, invoiceItems: _invoiceItems),
    );
    ...
    _promptForQuantity(selectedProduct, selectedVariant);
  }
  
  Future<void> _promptForQuantity(Product parentProduct, ProductVariant variant) async {
    final qtyController = TextEditingController(text: '1');
    final int qtyAlreadyInInvoice = _invoiceItems
        .where((item) => item.variant.id == variant.id)
        .fold(0, (sum, item) => sum + item.quantity);
    final int availableQty = variant.quantity - qtyAlreadyInInvoice;

    if (availableQty <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('المنتج "${parentProduct.name} (${variant.displayName})" نفذت كميته أو كل الكمية المتاحة مضافة بالفعل.', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo'))),
      );
      return;
    }
    ...
    final selectedQuantity = await showDialog<int>(
      context: context,
      builder: (ctx) => _QuantityPickerDialog(
    ...
    if (selectedQuantity != null && selectedQuantity > 0) {
      _addItemToInvoice(_ProvisionalSaleItem(
  ...
  Future<void> _saveInvoice() async {
    ...
    final List<db_invoice_item.InvoiceItem> finalInvoiceItems = _invoiceItems.map((item) {
      return db_invoice_item.InvoiceItem(
        ...
        itemTotal: item.itemTotal, category: item.parentProduct.category,
      );
    }).toList();
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Expanded(child: Text(item.displayName, style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Cairo', color: Theme.of(ctx).textTheme.bodyLarge?.color), textAlign: TextAlign.right)),
                    IconButton(
...
// --- NEW HELPER WIDGETS to simplify the dialogs ---

class _ProductPickerDialog extends StatefulWidget {
  final ProductProvider productProvider;
  final List<_ProvisionalSaleItem> invoiceItems;

  const _ProductPickerDialog({required this.productProvider, required this.invoiceItems});
...
class _ProductPickerDialogState extends State<_ProductPickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
              ...
              child: filteredProducts.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final totalQtyInInvoice = widget.invoiceItems
                          .where((item) => item.parentProduct.id == product.id)
                          .fold(0, (sum, item) => sum + item.quantity);
                      final canAdd = product.totalQuantity > totalQtyInInvoice;
                      return ListTile(
                        title: Text(product.name, textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo')),
                        // --- THIS IS THE FIX ---
                        subtitle: Text('الصنف: ${product.category} - المتاح: ${product.totalQuantity - totalQtyInInvoice}', textAlign: TextAlign.right, style: const TextStyle(fontSize: 12, fontFamily: 'Cairo')),
                        onTap: canAdd ? () => Navigator.of(context).pop(product) : null,
                        enabled: canAdd,
      ...
      actions: [ TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text("إلغاء", style: TextStyle(fontFamily: 'Cairo')))],
    );
  }
}
      ...
      content: SizedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final canAdd = variant.quantity > 0;
            return ListTile(
              title: Text(variant.displayName, textAlign: TextAlign.right),
              subtitle: Text('المتاح: ${variant.quantity}', textAlign: TextAlign.right),
              onTap: canAdd ? () => Navigator.of(context).pop(variant) : null,
              enabled: canAdd,
            );
          },
        ),
...
class _QuantityPickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text('أدخل الكمية لـ $productName (${variantName.isEmpty ? "" : variantName})', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Cairo')),
      content: Form(
        key: formKey,
        child: TextFormField(
          ...
          textAlign: TextAlign.right,
          style: const TextStyle(fontFamily: 'Cairo'),
          decoration: InputDecoration(hintText: 'الكمية (المتاح: $availableQty)', hintStyle: const TextStyle(fontFamily: 'Cairo', fontSize: 12)),
          autofocus: true,
          validator: (value) {
      ...
      actions: [
        TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text("إلغاء", style: TextStyle(fontFamily: 'Cairo'))),
        ElevatedButton(
          onPressed: () {
