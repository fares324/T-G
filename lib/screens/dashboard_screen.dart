...
import 'package:fouad_stock/screens/price_quote_screen.dart'; // Import the new screen
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _fetchDashboardData() async {
    ...
    try {
      ...
      await invoiceProvider.fetchInvoices(filter: InvoiceListFilter.none); 
    ...
    return Column(
      ...
      children: [
        ...
        GridView.count(
          ...
          childAspectRatio: MediaQuery.of(context).size.width > 650 ? 1.4 : 1.5,
          children: <Widget>[
...
// class _DashboardScreenState extends State<DashboardScreen> {
//   bool _isLoading = true; // For the dashboard's own data fetching cycle
...
//   Future<void> _fetchDashboardData() async {
//     if (!mounted) return;
//     // Set loading state at the beginning of this specific fetch operation
//     setState(() { _isLoading = true; });

//     try {
//       // Use listen:false because we are in a method, not directly in build,
//       // and we will manually update state via setState.
//       final productProvider = Provider.of<ProductProvider>(context, listen: false);
//       final invoiceProvider = Provider.of<InvoiceProvider>(context, listen: false);

//       // Fetch base data. These provider methods handle their own internal notifications
//       // if their lists are being directly observed elsewhere. For dashboard-specific KPIs,
//       // we await them and then use their summary methods.
//       await productProvider.fetchProducts(filter: ProductListFilter.none); 
//       await invoiceProvider.fetchInvoices(filter: InvoiceListFilter.none); 

//       // Now fetch specific summary data using await
//       final salesSummary = await invoiceProvider.getTodaysSalesSummary();
//       final purchasesSummary = await invoiceProvider.getTodaysPurchasesSummary();
//       final double unpaidAmount = await invoiceProvider.getTotalUnpaidSalesAmount(); 
...
//           ],
//         ),
//         body: Builder( // Use Builder to get a context that is below the Theme widget for Consumer
//           builder: (context) { // This context can now be used by Consumer
//             // We watch ProductProvider for inventory metrics, InvoiceProvider isn't directly used for list building here
//             final productProvider = context.watch<ProductProvider>(); 

//             // The dashboard's _isLoading state controls the main loader for KPI data
//             return _isLoading 
//                 ? Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(kpiCardColor1)))
//                 : RefreshIndicator(
//                     onRefresh: _fetchDashboardData,
//                     color: kpiCardColor1,
//                     backgroundColor: Theme.of(context).cardColor, // Use context for theme
//                     child: ListView(
//                       padding: const EdgeInsets.symmetric(horizontal: 18.0, vertical: 18.0),
//                       children: <Widget>[
//                         _buildKpiSection(context), 
//                         const SizedBox(height: 28),
//                         _buildSectionTitle(context, 'نظرة عامة على المخزون'), 
//                         _buildInventoryMetricsGrid(context, productProvider, Theme.of(context).cardColor), // Pass productProvider
//                         const SizedBox(height: 28),
//                         _buildSectionTitle(context, 'إجراءات سريعة'), 
//                         _buildQuickActions(context), 
//                         const SizedBox(height: 20),
...
//     // Show loading card for inventory if productProvider is loading its initial full list
//     if (productProvider.isLoading && productProvider.products.isEmpty && productProvider.currentFilter == ProductListFilter.none) {
//          cards.add(_buildSummaryCard(context: context, title: 'تحميل المخزون...', value: "...", icon: Icons.hourglass_empty, cardColor: cardBgColor, valueColor: darkSecondaryTextColor));
//     }
//     return GridView.builder(
...
//       { 'label': 'عرض كل الفواتير', 'icon': Icons.article_outlined, 'onPressed': () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => InvoicesListScreen(filter: InvoiceListFilter.none, appBarTitle: 'كل الفواتير'))).then((_) { if (mounted) _fetchDashboardData(); }), 'iconColor': kpiCardColor4, },
//       { 'label': 'تقرير المبيعات', 'icon': Icons.assessment_outlined, 'onPressed': () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const SalesReportScreen())).then((_) { if (mounted) _fetchDashboardData(); }), 'iconColor': kpiCardColor3, },
//       // In DashboardScreen.dart, within the 'actions' list:
// {
//   'label': 'تقرير المخزون',
//   'icon': Icons.assessment_outlined, // Or Icons.inventory, etc.
//   'onPressed': () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const InventoryReportScreen())),
//   // 'iconColor': someColor, // Choose a color
// },
//     ];
